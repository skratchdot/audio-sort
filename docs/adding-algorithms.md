...
1. Add `src/js/sort/sort.<id>.mjs`, following an existing algorithm's default
...
2. Import and register the function under a stable ID in
   [`registry.mjs`](../src/js/sort/registry.mjs).
3. Add a raw-source import and matching entry in
   [`sources.mjs`](../src/js/sort/sources.mjs) for the editor.
...
Tests discover `sort.*.mjs` files and check registry/source coverage, sorting
